...
export default function WorkoutProgramCard({ workoutProgramData, memberId }) {
        ...
        <CardContent className="flex items-center justify-center py-8">
          <div className="text-center">
            ...
            <Button size="sm" className="mt-4" asChild>
              <Link href={`/member/${memberId}/workout`}>
                ดูโปรแกรมทั้งหมด
              </Link>
            </Button>
          </div>
  ...
  const sortedPrograms = workoutProgramData.data.availablePrograms?.sort(
    ...
    return (
      <Card className="w-full min-h-[120px]">
        <CardContent className="flex items-center justify-center py-8">
          <div className="text-center">
            <div className="text-4xl mb-2">📋</div>
            <p className="text-gray-500 text-sm">ยังไม่มีโปรแกรมสำหรับวันนี้</p>
          </div>
        </CardContent>
      </Card>
    );
