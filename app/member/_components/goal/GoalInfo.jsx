...
export default function GoalInfo({
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground">
            ยังไม่มีเป้าหมายการออกกำลังกาย กรุณาตั้งเป้าหมายของคุณ
          </p>
        </CardContent>
      </Card>
    );
  ...
  const calculateProgress = () => {
    ...
    if (
      (goalData.goal_type === "ลดน้ำหนัก" &&
        currentWeightValue > targetWeight) ||
      (goalData.goal_type === "เพิ่มน้ำหนัก" &&
        currentWeightValue < targetWeight)
    ) {
  ...
  const handleStatusChange = async (status) => {
    if (!confirm(`คุณต้องการเปลี่ยนสถานะเป้าหมายเป็น ${status} ใช่หรือไม่?`))
      return;

    setLoading(true);
    try {
      const result = await changeMemberGoalStatus(
        goalData.fitness_goal_id,
        status,
        goalData.member_id
      );
      toast({
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const result = await deleteMemberGoal(
        goalData.fitness_goal_id,
        goalData.member_id
      );
      toast({
  ...
  return (
    <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            ...
            {(goalData.fitness_goal_type === "ลดน้ำหนัก" ||
              goalData.fitness_goal_type === "เพิ่มน้ำหนัก" ||
              goalData.fitness_goal_type === "คงที่น้ำหนัก") && (
              <div className="text-center space-y-2">
                ...
                {currentWeight && (
                  <div className="text-center mt-2">
                    <p className="text-sm">
                      น้ำหนักปัจจุบัน:{" "}
                      <span className="font-medium">{currentWeight} กก.</span>
                    </p>
                    <p className="text-sm">
                      {progress.remaining > 0
                        ? `ต้อง${
                            goalData.goal_type === "ลดน้ำหนัก" ? "ลด" : "เพิ่ม"
                          }อีก ${progress.remaining} กก.`
                        : "บรรลุเป้าหมายแล้ว!"}
                    </p>
                  </div>
                )}
      ...
      <CardFooter className="flex justify-end space-x-2">
        ...
        <Button size="sm" onClick={() => onEdit(goalData)} disabled={loading}>
          แก้ไข
        </Button>
      </CardFooter>
    </Card>
