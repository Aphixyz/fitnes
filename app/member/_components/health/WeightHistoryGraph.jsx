...
export default function WeightHistoryGraph({ historyData }) {
  ...
  useEffect(() => {
    ...
    const filtered = historyData
      .filter(record => new Date(record.member_health_measurementdate) >= startDate)
      .sort((a, b) => new Date(a.member_health_measurementdate) - new Date(b.member_health_measurementdate));
      ...
      <CardContent className="pt-4">
        {filteredData.length === 0 ? (
          <div className="flex justify-center items-center h-64">
            <p className="text-muted-foreground">ไม่มีข้อมูลในช่วงเวลาที่เลือก</p>
          </div>
        ) : (
          <div className="h-64">
            <LineChart
              data={filteredData.map(record => {
                ...
                return {
                  ...
                  timestamp: new Date(record.member_health_measurementdate).getTime()
                };
              })}
            />
          </div>
...
function renderLineGraph(data, field) {
  ...
  let path = `M ${calculateXPosition(0, data.length)} ${calculateYPosition(data[0][field], data, field)}`;
  
  for (let i = 1; i < data.length; i++) {
    path += ` L ${calculateXPosition(i, data.length)} ${calculateYPosition(data[i][field], data, field)}`;
  }
