...
export async function GET(req, context) {
    try {
        ...
        const [result] = await pool.query("SELECT * FROM member_health WHERE member_id = ?", [id]);
...
export async function POST(req, context) {
    try {
      ...
      if (!member_health_weight || !member_health_height || !member_health_bmi) {
        return NextResponse.json({ error: "Weight, height, and BMI are required" }, { status: 422 });
      }
      ...
      if (!result || result.affectedRows === 0) {
        return NextResponse.json({ error: "Insert failed, no rows affected" }, { status: 500 });
      }
