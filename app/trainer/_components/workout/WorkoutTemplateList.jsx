...
import { createWorkoutFromTemplate } from "@/schemas/workoutv1/workoutTemplateActions";
...
export default function WorkoutTemplateList({
  ...
  const handleView = (templateId) => {
    if (onSelect) {
      ...
      router.push(`/trainer/${trainerId}/workout/templates/${templateId}`);
    }
  };
  ...
  const handleUseTemplate = async (templateId, memberIdParam) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push(`/trainer/${trainerId}/workout/${result.workout_plan_id}`);
      } else {
        toast({
            ...
            <CardHeader className="pb-2">
              <div className="flex justify-between">
                ...
                <DropdownMenu>
                  ...
                  >
                    <DropdownMenuItem
                      onClick={() => handleView(template.template_id)}
                    >
                      <Eye className="mr-2 h-4 w-4" /> ดูรายละเอียด
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={() =>
                        handleUseTemplate(template.template_id, memberId)
                      }
                      disabled={loading === template.template_id}
                    >
                      <Copy className="mr-2 h-4 w-4" /> ใช้เทมเพลตนี้
                    </DropdownMenuItem>
                    {template.trainer_id.toString() ===
                      trainerId.toString() && (
                      <>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem
                          onClick={() =>
                            router.push(
                              `/trainer/${trainerId}/workout/templates/edit/${template.template_id}`
                            )
                          }
                        >
