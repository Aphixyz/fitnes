...
import { createWorkoutPlan, updateWorkoutPlan } from "@/actions/trainer/(workout)/workoutPlanActions";
...
export default function WorkoutPlanForm({ trainerId, initialData, memberId }) {
  ...
  const [formData, setFormData] = useState({
    ...
    plan_startdate: initialData?.plan_startdate ? new Date(initialData.plan_startdate).toISOString().split("T")[0] : new Date().toISOString().split("T")[0],
    plan_enddate: initialData?.plan_enddate ? new Date(initialData.plan_enddate).toISOString().split("T")[0] : "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        result = await updateWorkoutPlan(initialData.workout_plan_id, updatedFormData, trainerId);
      } else {
      ...
      if (result.success) {
        toast({
          title: "สำเร็จ",
          description: isEditing ? "อัพเดตโปรแกรมการฝึกสำเร็จ" : "สร้างโปรแกรมการฝึกสำเร็จ",
        });
        ...
        if (isEditing) {
          ...
          router.push(`/trainer/${trainerId}/workouts/${result.plan_id}/exercises`);
        }
      } else {
        toast({
          title: "เกิดข้อผิดพลาด",
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Card>
        <CardHeader>
          <CardTitle>{isEditing ? "แก้ไขโปรแกรมการฝึก" : "สร้างโปรแกรมการฝึกใหม่"}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {/* ชื่อโปรแกรม */}
            <div className="space-y-2">
              <Label htmlFor="plan_name">ชื่อโปรแกรม <span className="text-red-500">*</span></Label>
              <Input
                id="plan_name"
                name="plan_name"
            ...
            {!memberId && (
              <div className="space-y-2">
                <Label htmlFor="member_id">สมาชิก <span className="text-red-500">*</span></Label>
                <Select
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => handleSelectChange("workout_frequency", parseInt(value))}
              >
            ...
            {/* วันที่เริ่มต้น */}
            <div className="space-y-2">
              <Label htmlFor="plan_startdate">วันที่เริ่มต้น <span className="text-red-500">*</span></Label>
              <Input
                id="plan_startdate"
                name="plan_startdate"
                type="date"
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">หากไม่ระบุ จะถือว่าไม่มีวันสิ้นสุด</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div>
              <Label>วันที่ออกกำลังกาย <span className="text-red-500">*</span></Label>
              {errors.workout_days && (
                <p className="text-xs text-red-500 mt-1">{errors.workout_days}</p>
              )}
            </div>
            
            <div className="grid grid-cols-2 md:grid-cols-7 gap-2">
              {weekDays.map((day) => (
                <div key={day.id} className="flex items-center space-x-2 border rounded p-3">
                  <Checkbox
                    id={`day-${day.id}`}
          ...
          {isEditing && (
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => handleSelectChange("plan_status", value)}
              >
          ...
          <Alert>
            ...
            <AlertDescription>
              หลังจากสร้างโปรแกรมการฝึก คุณจะสามารถเพิ่มท่าออกกำลังกายได้ในขั้นตอนถัดไป
            </AlertDescription>
          </Alert>
        </CardContent>
        <CardFooter className="flex justify-between">
          ...
          <Button 
            ...
            disabled={submitting}
          >
            {submitting ? "กำลังบันทึก..." : isEditing ? "อัพเดต" : "สร้างโปรแกรม"}
          </Button>
        </CardFooter>
      </Card>
    </form>
