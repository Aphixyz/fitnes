...
import { 
  ...
  MoveVertical, ChevronRight, ChevronLeft
} from "lucide-react";
import { toast } from "@/components/ui/use-toast";
import { deletePlanExercise, moveExerciseToDay } from "@/actions/trainer/(workout)/workoutExerciseActions";
...
export default function ExerciseList({ 
  ...
  const [activeTab, setActiveTab] = useState(Object.keys(groupedByDay)[0] || "none");
  ...
  const handleEditExercise = (exerciseId) => {
    router.push(`/trainer/${trainerId}/workouts/${planId}/exercises/${exerciseId}`);
  };
    ...
    return (
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          {currentDay !== "none" && (
            <DropdownMenuItem
              onClick={() => handleMoveToDay(exercise.workout_exercise_id, "none")}
              disabled={loading}
              className="cursor-pointer"
            >
              ไม่ระบุวัน
      ...
      {exercises && exercises.length > 0 ? (
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 lg:flex">
            {sortedDays().map(day => (
              <TabsTrigger key={day} value={day} className="text-center whitespace-nowrap">
                {getDayName(day)}
              </TabsTrigger>
            ))}
          </TabsList>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex justify-between items-center">
                    ...
                    <Button onClick={() => router.push(`/trainer/${trainerId}/workouts/${planId}/exercises/add?day=${day}`)}>
                      เพิ่มท่าออกกำลังกาย
                    </Button>
                  </CardTitle>
                  <CardDescription>
                      ...
                      <TableBody>
                        {groupedByDay[day].map((exercise, index) => (
                          <TableRow key={exercise.workout_exercise_id}>
                            <TableCell>{exercise.exercise_order || index + 1}</TableCell>
                            ...
                            <TableCell>{exercise.rest_seconds ? `${exercise.rest_seconds} วินาที` : "-"}</TableCell>
                            <TableCell className="text-right">
                              <div className="flex justify-end items-center gap-1">
                                ...
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  onClick={() => handleEditExercise(exercise.workout_exercise_id)}
                                  disabled={loading}
                                >
                                ...
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  onClick={() => handleDeleteExercise(exercise.workout_exercise_id)}
                                  disabled={loading}
                                >
                ...
                <CardFooter className="flex justify-end">
                  <Button 
                    onClick={() => router.push(`/trainer/${trainerId}/workouts/${planId}/exercises/add?day=${day}`)}
                  >
                    เพิ่มท่าออกกำลังกาย
                  </Button>
                </CardFooter>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>ยังไม่มีท่าออกกำลังกายในโปรแกรมนี้</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-center py-6">
              <p className="mb-4 text-muted-foreground">คุณยังไม่ได้เพิ่มท่าออกกำลังกายในโปรแกรมนี้</p>
              <Button 
                onClick={() => router.push(`/trainer/${trainerId}/workouts/${planId}/exercises/add`)}
              >
                เพิ่มท่าออกกำลังกาย
              </Button>
            </div>
