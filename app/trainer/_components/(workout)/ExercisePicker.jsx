...
export default function ExercisePicker({ onSelect }) {
  ...
  const extractFilterOptions = (exercisesList) => {
    ...
    setMuscleGroups([
      { value: "all", label: "ทั้งหมด" },
      ...Array.from(muscleGroupSet).sort().map(muscle => ({
        value: muscle,
    ...
    setEquipmentList([
      { value: "all", label: "ทั้งหมด" },
      ...Array.from(equipmentSet).sort().map(equip => ({
        value: equip,
  ...
  const filteredExercises = exercises.filter(exercise => {
    ...
    const muscleMatch = muscleGroup === "all" || 
    ...
    // กรองตามอุปกรณ์
    const equipmentMatch = equipment === "all" || 
      (exercise.equipment && exercise.equipment.includes(equipment));
    
    return searchMatch && categoryMatch && muscleMatch && difficultyMatch && equipmentMatch;
  });
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-grow">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {exercise.muscle_groups && exercise.muscle_groups.map((muscle) => (
                          <Badge key={muscle} variant="outline" className="bg-blue-50">
                            {muscle}
                          </Badge>
                        ))}
                      </div>
                  ...
                  <div className="flex mt-2 text-xs text-muted-foreground">
                    <div className="flex items-center mr-3">
                      ...
                      <span className="capitalize">{exercise.difficulty || "beginner"}</span>
                    </div>
