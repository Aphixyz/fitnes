...
import { getWorkoutTemplateById, createWorkoutFromTemplate } from "@/actions/trainer/(workout)/workoutTemplateActions";
...
import { ChevronLeft, Calendar, Users, Dumbbell, Copy, Edit, Clock, BookOpen, User } from "lucide-react";
        ...
        <Card>
          <CardContent className="py-10">
            <div className="text-center">
              <p className="text-muted-foreground">ไม่พบข้อมูลเทมเพลตโปรแกรมการฝึก</p>
            </div>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight mb-1">{template.template_name}</h1>
        ...
        <div className="flex space-x-2">
          ...
          {template.trainer_id.toString() === trainerId.toString() && (
            <Button 
              variant="outline"
              onClick={() => router.push(`/trainer/${trainerId}/workouts/templates/edit/${templateId}`)}
            >
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <h3 className="text-sm font-medium text-muted-foreground">ข้อมูลเทมเพลต</h3>
                  <div className="mt-2 space-y-2">
                    ...
                    <div className="flex justify-between py-1 border-b">
                      <span className="font-medium">กลุ่มกล้ามเนื้อเป้าหมาย:</span>
                      <span>{template.target_muscles || "-"}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b">
                      <span className="font-medium">สถานะ:</span>
                      <span>{template.is_public === 1 ? "สาธารณะ" : "ส่วนตัว"}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b">
                ...
                <div>
                  ...
                  <div className="mt-2 p-4 bg-gray-50 rounded-md border">
                    <p className="whitespace-pre-line">{template.template_description || "ไม่มีรายละเอียดเพิ่มเติม"}</p>
                  </div>
                </div>
              </div>
