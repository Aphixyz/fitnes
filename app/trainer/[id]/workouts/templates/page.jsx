    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          ...
          description: result.message || "ไม่สามารถดึงข้อมูลเทมเพลตโปรแกรมการฝึกได้",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const getFilteredTemplates = () => {
    return templates.filter(template => {
      ...
      const matchesSearch = 
        ...
        (template.template_description && template.template_description.toLowerCase().includes(searchLower)) ||
        template.trainer_name.toLowerCase().includes(searchLower);
      
      if (activeTab === "all") {
        return matchesSearch;
      } else if (activeTab === "my") {
        return matchesSearch && template.trainer_id.toString() === trainerId.toString();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight mb-1">เทมเพลตโปรแกรมการฝึก</h1>
      ...
      <form onSubmit={handleSearch} className="flex items-center max-w-xl">
        ...
        <Button type="submit" className="ml-2">ค้นหา</Button>
      </form>
